...
export class PPLBCompiler implements PrinterCompiler {
  ...
  validate(document: LabelDocument): ValidationResult {
    ...
    if (document.elements && document.elements.some((elem) => elem.type === 'image' && elem.visible !== false)) {
      errors.push('Este modelo contém uma imagem, mas a linguagem PPLB selecionada ainda não possui suporte a bitmap.');
    }
  ...
  compile(document: LabelDocument, data: Record<string, string> = {}): CompiledLabel {
    ...
    document.elements.forEach((elem: LabelElement) => {
      ...
      switch (elem.type) {
        ...
        case 'image': {
          throw new Error('Este modelo contém uma imagem, mas a linguagem PPLB selecionada ainda não possui suporte a bitmap.');
        }
