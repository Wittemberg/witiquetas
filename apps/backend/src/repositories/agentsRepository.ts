...
export class AgentsRepository {
  ...
  static async save(agent: AgentRecord): Promise<void> {
    ...
    try {
      ...
      console.error(`[AgentsRepository] Erro ao persistir agente no PostgreSQL: ${err.message}`);
    }
  }
  ...
  static async findById(id: string): Promise<AgentRecord | null> {
    ...
    try {
      ...
      console.error(`[AgentsRepository] Erro ao buscar agente por ID: ${err.message}`);
      return null;
    }
  }
  ...
  static async findByTokenHash(tokenHash: string): Promise<AgentRecord | null> {
    ...
    // 2. Consulta fonte da verdade (PostgreSQL)
    try {
      const res = await pgPool.query(
      ...
      console.error(`[AgentsRepository] Erro ao buscar agente por tokenHash: ${err.message}`);
      return null;
    }
  }
